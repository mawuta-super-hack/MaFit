...
def workouts(call):
    ...
    markup = types.ReplyKeyboardMarkup(
        resize_keyboard=True).add('Вернуться в начало')
...
def exercise(call):
    ...
    markup = types.ReplyKeyboardMarkup(
        resize_keyboard=True).add('Вернуться в начало')
    bot.send_message(
        call.chat.id, 'Есть ли у Вас еще вопросы?', reply_markup=markup
    )
    bot.register_next_step_handler(call, start)
...
def on_click(call):
    if call.text == 'Показать несколько советов':
        ...
        markup = types.ReplyKeyboardMarkup(
            resize_keyboard=True).add('Вернуться в начало')
        bot.send_message(
            call.chat.id, 'Этo ценная информация!', reply_markup=markup
        )
        bot.register_next_step_handler(call, start)
        ...
        bot.send_message(
            call.chat.id, 'Пожалуйста, выберите вид тренировки:',
            reply_markup=markup
        )
        ...
        bot.send_message(
            call.chat.id, 'Пожалуйста, веберите группу мышц:',
            reply_markup=markup
        )
